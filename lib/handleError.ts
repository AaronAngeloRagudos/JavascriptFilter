    ...
    if (matchTo.length >= 1) {
        for (i = 0; i < matchTo.length; i++) {
            if (typeof matchTo[i] !== 'string') {
                return {
                    status: true,
                    message: `This filter only accepts an array[] of strings for preferences. Please refactor the preferences. The preferences: ${matchTo}`,
                    type: 'TypeError'
                }
            }
        }
        ...
        for (j = 0; j < dataToBeMatched.length; j++) {
            ...
            if (typeof dataToBeMatched[j] === 'object') {
                ...
                if (dataToBeMatched[j].constructor !== Array) {
                    return {
                        status: true,
                        message: `This filter only accepts an array[] that contains an array[] of strings for data comparison. Please refactor the array[][] of data you want to compare with the preferences. The data: ${dataToBeMatched}`,
                        type: 'TypeError'
                    }
                }
            } else {
